...
if (day_int%2)==1 :
    ...
    print "growLight on"
    draw2.text((x, top+20), 'Grow Light on..',  font=font, fill=255)
    disp.image(image2)
    disp.display()
    ...
    with open('/home/pi/Deep_data/growLight_status.csv', mode='w') as csv_file:
        ...
        writer.writeheader()
        writer.writerow({'timeStamp': currentdate,'growLight': '1'})
        
    logging.info('Grow Light turned On')

else :
    logging.info('Even day, Grow light is turned off')
    draw2.text((x, top+20), 'Even day, Grow',  font=font, fill=255)
    draw2.text((x, top+30), 'light turned off',  font=font, fill=255)
    disp.image(image2)
    disp.display()
